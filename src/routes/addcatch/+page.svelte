<script>
    ...
    const handleOnSubmit = async (event) => {
        ...
        if (localStorage.getItem("fishes") === "") {
            fishes.set(JSON.stringify({ [uuid]: fish }));
            modal = true;
            fish = { ...valueDefaults };
        } else {
    ...
    onMount(async function () {
        ...
        fetchedFishes = Object.keys(data)
            .map((x) => {
                return {
    ...
    <form on:submit={handleOnSubmit}>
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            <Label>
                ...
                <Select
                    class="mt-2"
                    bind:items={fetchedFishes}
                    bind:value={fish.fishType}
                    required
                />
            </Label>
            <div>
    ...
    <Modal title={"Great catch!"} bind:open={modal} autoclose>
        <div class="flex">
            ...
            <div class="ml-3 text-md font-normal">
                <span
                    class="mb-1 text-md font-semibold text-gray-900 dark:text-white"
                    >You have now registered the fish:</span
                >
